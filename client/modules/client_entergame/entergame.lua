...
local function onMotd(protocol, motd)
    -- server may send the message of the day without a numeric prefix
    local newlineIndex = motd:find('\n')
    if newlineIndex then
        G.motdNumber = tonumber(motd:sub(1, newlineIndex - 1)) or 0
        G.motdMessage = motd:sub(newlineIndex + 1)
    else
        G.motdNumber = 0
        G.motdMessage = motd
    end
